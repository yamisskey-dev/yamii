...
from .base_bot import BaseBotConfig
...
class NaviAPIClient:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=self.config.navi_api_timeout)
        )
        return self
    ...
    async def health_check(self) -> Dict[str, Any]:
        """naviサーバーのヘルスチェック"""
        try:
            url = f"{self.config.navi_api_url}/health"
    ...
    async def send_counseling_request(self, request: NaviRequest) -> Optional[NaviResponse]:
        """人生相談リクエストを送信"""
        try:
            url = f"{self.config.navi_api_url}/counseling"
    ...
    async def get_custom_prompt(self, user_id: str) -> Dict[str, Any]:
        """カスタムプロンプトを取得"""
        try:
            url = f"{self.config.navi_api_url}/custom-prompts"
    ...
    async def get_user_profile(self, user_id: str) -> Optional[Dict[str, Any]]:
        """ユーザープロファイルを取得"""
        try:
            url = f"{self.config.navi_api_url}/profile"
            params = {"user_id": user_id}
