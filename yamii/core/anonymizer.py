...
class PIIAnonymizer:
    ...
    def __init__(self):
        ...
        self._patterns: list[tuple[str, str, re.Pattern]] = [
            ...
            # 電話番号（携帯・固定）
            ("PHONE", "電話番号",
             re.compile(r'(?:0\d{1,4}[-\s]?\d{1,4}[-\s]?\d{3,4}|\d{3}[-\s]?\d{4}[-\s]?\d{4})')),
            ...
            # 住所（都道府県から始まる）
            ("ADDRESS", "住所",
             re.compile(r'(?:東京都|北海道|(?:京都|大阪)府|.{2,3}県)[^\s。、]+?(?:\d+[-ー]\d+[-ー]?\d*|丁目|番地?|号)')),
        ]

        # 日本人名パターン（姓名の組み合わせ）
        self._name_patterns = [
            # 「〜さん」「〜様」「〜君」「〜ちゃん」で終わる名前
            re.compile(r'([一-龯ぁ-んァ-ン]{1,4})\s*([一-龯ぁ-んァ-ン]{1,4})\s*(?:さん|様|君|ちゃん|先生|氏)'),
            # 「私は〜です」パターン
            re.compile(r'(?:私は|僕は|俺は|名前は)\s*([一-龯ぁ-んァ-ン]{2,8})(?:です|と申します|といいます)'),
        ]
