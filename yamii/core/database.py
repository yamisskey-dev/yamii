...
class DatabaseConfig:
    ...
    def from_env(cls) -> 'DatabaseConfig':
        """環境変数からの設定読み込み"""
        return cls(
            host=os.getenv("YAMII_DB_HOST", "localhost"),
            port=int(os.getenv("YAMII_DB_PORT", "5432")),
            database=os.getenv("YAMII_DB_NAME", "yamii"),
            user=os.getenv("YAMII_DB_USER", "yamii_user"),
            password=os.getenv("YAMII_DB_PASSWORD", "yamii_password"),
            pool_size=int(os.getenv("YAMII_DB_POOL_SIZE", "10")),
            max_overflow=int(os.getenv("YAMII_DB_MAX_OVERFLOW", "20")),
            echo=os.getenv("YAMII_DB_ECHO", "false").lower() == "true"
        )
...
class AsyncDatabase:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise YamiiException(f"Database initialization failed: {e}")
    ...
    async def create_tables(self) -> None:
        ...
        try:
            ...
            raise YamiiException(f"Table creation failed: {e}")
    ...
    async def drop_tables(self) -> None:
        ...
        try:
            ...
            raise YamiiException(f"Table dropping failed: {e}")
    
    @asynccontextmanager
    async def get_session(self) -> AsyncGenerator[AsyncSession, None]:
        """セッション取得コンテキストマネージャー"""
        if not self.session_maker:
            raise YamiiException("Database not initialized")
