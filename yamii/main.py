...
from .core.logging import YamiiLogger, get_logger, log_request, log_response, log_error
from .core.exceptions import YamiiException, ExternalServiceError, ValidationError
    ...
    if _bot_enabled and _bot_task is None:
        try:
            ...
            if enable_bot:
                ...
                # 遅延インポートで循環参照を完全に回避
                from .bot.misskey.yamii_bot import YamiiMisskeyBot, load_config

                bot_config = load_config()
                bot = YamiiMisskeyBot(bot_config)
...
# エラーハンドラー
@app.exception_handler(YamiiException)
async def yamii_exception_handler(request, exc: YamiiException):
...
async def startup_event():
    """アプリケーション起動時処理"""
    logger.info("Yamii APIサーバーを起動中...")
    await start_misskey_bot()
    logger.info("Yamii APIサーバー起動完了")

@app.on_event("shutdown")
async def shutdown_event():
    """アプリケーション終了時処理"""
    logger.info("Yamii APIサーバーを終了中...")
    await stop_misskey_bot()
    logger.info("Yamii APIサーバー終了完了")
