...
yui の yamii モジュールを Python で再実装したもの

## Overview

このボットは、MisskeyのSNS上で人生相談を行う AI ボットです。
TypeScript で書かれた元の yui yamii モジュールの機能を、Python で完全に再実装しています。
Yamii API サーバーと統合することで、統一されたカウンセリングサービスを提供します。
...
# Yamii API設定
BOT_YAMII_API_URL=http://localhost:8000
BOT_YAMII_API_TIMEOUT=30
...
```python
from yamii.bot.misskey import YamiiBot
import asyncio
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Misskey SNS   │◄───┤  Yamii Bot       │◄───┤  Yamii API       │
│                 │    │                 │    │  Server         │
...
- **YamiiBot**: メインボットクラス - WebSocket接続とイベント処理
- **MisskeyClient**: Misskey API クライアント - API通信とWebSocket管理  
- **YamiiClient**: Yamii API クライアント - カウンセリング機能との連携
...
3. **メッセージ処理**: 
   - ユーザーのメッセージを受信
   - Yamii API でカウンセリングレスポンスを生成
   - Misskey にリプライまたはノート作成
4. **セッション管理**: ユーザーごとの会話履歴を保持・管理

## API Integration

Yamii API サーバーと連携して以下の機能を使用:
...
# 設定検証
python -c "from yamii.bot.misskey.config import load_config; print(load_config())"
```
...
```
yamii/
├── yamii/                      # メインPythonパッケージ
│   ├── main.py               # Yamiiサーバー（ボット統合対応）
│   ├── core/, services/      # 既存のyamii機能
│   └── bot/                  # ボット機能
│       ├── __init__.py
│       ├── README.md         # このファイル
...
│           ├── yamii_client.py     # Yamii API クライアント
│           └── yamii_bot.py        # メインボット実装（388行）
├── start_yamii.py                  # 統合サーバー起動スクリプト
├── run_yamii.sh                   # シェル起動スクリプト（実行可能）
...
2. **統合サーバーが起動しない**
   - Yamiiサーバーの依存関係がインストールされているか確認
   - `python start_yamii.py` または `./run_yamii.sh` で起動
...
# APIサーバーログと合わせて確認
python start_yamii.py  # ターミナル出力で確認