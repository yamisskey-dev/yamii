...
from .yamii_api_client import YamiiAPIClient, YamiiRequest
...
class MessageHandler:
    ...
    async def process_message(
        ...
        try:
            ...
            if command.command_type == BotCommandType.HELP:
                ...
                await self._handle_status_command(context, command, yamii_client, bot)
            elif command.command_type == BotCommandType.CUSTOM_PROMPT:
                await self._handle_custom_prompt_command(context, command, yamii_client, bot)
            elif command.command_type == BotCommandType.PROFILE:
                await self._handle_profile_command(context, command, yamii_client, bot)
                ...
                await self._handle_counseling_command(context, command, yamii_client, session_manager, bot)
            else:
    ...
    async def _handle_status_command(
        ...
        """ステータスコマンド処理"""
        try:
            health = await yamii_client.health_check()
    ...
    async def _handle_custom_prompt_command(
        ...
        try:
            if command.action == "show":
                prompt_data = await yamii_client.get_custom_prompt(context.user_id)
                ...
                success = await yamii_client.delete_custom_prompt(context.user_id)
                ...
                if command.is_valid and command.content:
                    success = await yamii_client.create_custom_prompt(context.user_id, command.content)
                    if success:
    ...
    async def _handle_profile_command(
        ...
        try:
            if command.action == "show":
                profile = await yamii_client.get_user_profile(context.user_id)
                ...
                success = await yamii_client.delete_user_profile(context.user_id)
                reply_text = "✅ プロファイルを削除しました。" if success else "❌ 削除するプロファイルが見つかりませんでした。"
                await bot.send_reply(context.message_id, reply_text)
                
            elif command.action == "set":
                if command.is_valid and command.content:
                    success = await yamii_client.set_user_profile(context.user_id, command.content)
                    if success:
                        reply_text = (
                            f"✅ プロファイルを設定しました。\n\n"
